...
@app.route('/<int:tyr_id>', methods=['POST', 'GET'])
def zak_tyr(tyr_id):
    if request.method == 'GET':
        db_session.global_init("db/database.sqlite")
        session = db_session.create_session()
        t = session.query(tyrs.Tyrs).filter(tyrs.Tyrs.id == tyr_id).first()
        session.commit()
        return render_template("zakaz.html", tyr=t)
    elif request.method == 'POST':
        zak = zakazy.Zakazy(
            Name=request.form['name'],
            Phone=request.form['telefon'],
            Kol_vo=request.form['kol_vo'],
            Tyr_id=tyr_id
        )
        db_session.global_init("db/database.sqlite")
        session = db_session.create_session()
